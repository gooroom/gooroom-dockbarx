...
ATOM_PREVIEWS = Gdk.Atom.intern("_KDE_WINDOW_PREVIEW", True)
...
class GroupButton(CairoAppButton):
    ...
    def __init__(self, group):
        ...
        self.drag_source_set(Gdk.ModifierType.BUTTON1_MASK,
                                    [Gtk.TargetEntry.new("text/groupbutton_name", 0, 47593)],
                                    Gdk.DragAction.MOVE)
    ...
    def apply_launch_effect(self, length=None):
        ...
        if group.popup.get_window() is not None:
            group.popup.get_window().set_cursor(Gdk.Cursor(Gdk.CursorType.WATCH))
    ...
    def do_drag_motion(self, drag_context, x, y, t):
        ...
        if "text/groupbutton_name" in drag_context.list_targets()[0].name() and \
    ...
    def do_drag_enter(self, drag_context, x, y, t):
        ...
        if not "text/groupbutton_name" in drag_context.list_targets()[0].name():
    ...
    def do_leave_notify_event(self, event):
        ...
        if self.pointer_is_inside():
            ...
            GLib.timeout_add(50, self.do_leave_notify_event, event)
            return
    ...
    def do_button_release_event(self, event):
        ...
        if int(event.button.button) == 1 and self.globals.dragging:
            self.globals.dragging = False
            return
...
class GroupPopup(CairoPopup):
    ...
    def do_size_allocate(self, allocation):
        ...
        i, wx, wy = group.button.get_window().get_origin()
    ...
    def __set_own_strut(self, *args):
        if self.window:
            if self.globals.settings["locked_list_no_overlap"] is False:
                self.window.property_get("_NET_WM_STRUT", pdelete=True)
                self.window.property_get("_NET_WM_STRUT_PARTIAL",
                                          pdelete=True)
                return
            group = self.group_r()
            a = self.get_allocation()
