...
ATOM_WM_CLASS = Gdk.Atom.intern("WM_CLASS", True)
...
class Spacer(Gtk.EventBox):
    ...
    def do_drag_drop(self, drag_context, x, y, t):
        if "text/groupbutton_name" in drag_context.list_targets()[0].name():
            self.drag_get_data(drag_context, drag_context.list_targets()[0], t)
...
class GroupList(list):
    def __init__(self, dockbar, orient):
        ...
        self.box.pack_start(self.container, False, True, 0)
    ...
    def append(self, group):
        ...
        self.container.pack_start(group.button, False, True, 0)
        self.manage_size_overflow()
    ...
    def manage_size_overflow(self):
        ...
        # FIXME: group hide 시 dockbar drag drop이 안됨
    ...
    def calculate_max_size(self):
        if self.max_size is None:
            ...
            if self.orient in ("down", "up"):
                return self.box.get_allocation().width
            else:
                return self.box.get_allocation().height
        else:
            size = self.max_size
...
class DockBar():
    def __init__(self, parent):
        ...
        self.globals.connect("refresh", self.__on_refresh)

    #### Parent functions
    # The dock/applet/widget interacts with dockbar through these functions.
    def __on_refresh(self, *args):
    ...
    def open_preference(self):
        ...
        os.spawnlp(os.P_NOWAIT,"/usr/bin/pyg_dbx_preference",
                   "/usr/bin/pyg_dbx_preference")
                   
    #### Menu
    def create_popup_menu(self, event):
        if self.parent_handles_menu:
            #self.parent.create_popup_menu(event)
            self.parent.create_popup_menu_ex(event)
            return
        POPUP_INFO = """
            <ui>
                <popup name='PopupMenu'>
                    <menuitem action='gtk-properties'/>
                    <menuitem action='gtk-refresh'/>
                    <menuitem action='gtk-close'/>
                </popup>
            </ui>
        """
        uimanager = Gtk.UIManager()
        uimanager.add_ui_from_string(POPUP_INFO)
        accgr = uimanager.get_accel_group()
        self.add_accel_group(accgr)

        menu = Gtk.Menu()
        action_group = Gtk.ActionGroup("popup_menu")
        action_group.add_actions([
            ("gtk-properties", None, "_Preference", None, "Open Properties",
            self.__menu_preferences_selected),
            ("gtk-refresh", None, "_Reload", None, "Refresh Dock Bar",
            self.__menu_reload_selected),
            ("gtk-close", None, "_Close", None, "Close Dock Bar",
            self.__menu_close_selected)
        ])
        uimanager.insert_action_group(action_group)
        popup = uimanager.get_widget("/PopupMenu")
        popup.popup(None, None, None, None, event.button.button, event.time)
        return

        menu = Gtk.Menu()
        menu.connect("selection-done", self.__menu_closed)
        preference_item = Gtk.ImageMenuItem.new("gtk-properties", "Preference")
