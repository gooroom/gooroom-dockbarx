...
class Theme(GObject.GObject):
    __gsignals__ = {
        "theme_reloaded": (GObject.SignalFlags.RUN_FIRST, GObject.TYPE_NONE,()),
    }
       ...
       # of the theme names and paths so that a theme can be loaded
        ...
        if not themes:
            md = Gtk.MessageDialog (None,
                                                        ...
                                                        text=_("No working themes found in /usr/share/dockbarx/themes or ~/.dockbarx/themes"))
            md.run()
            md.destroy()
...
class DockTheme(GObject.GObject):
    ...
    def reload(self):
        if self.theme_path is None:
            return
        self.default_colors = {"bg-color": "#111111", "bg-alpha": 127,
                               "bar2-bg-color":"#111111", "bar2-bg-alpha": 127}
        try:
            tar = taropen(self.theme_path)
        except:
