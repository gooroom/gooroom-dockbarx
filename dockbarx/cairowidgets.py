...
class CairoAppButton(Gtk.EventBox):
    ...
    def __init__(self, surface=None, expose_on_clear=False):
        ...
        self.area = Gtk.Alignment()
        self.area.set(0, 0, 1, 1)
        self.add(self.area)
        self.area.show()
    ...
    def update(self, surface=None):
        ...
        self.get_window().invalidate_rect(a, True)
...
class CairoSmallButton(Gtk.Button):
    ...
    def do_enter_notify_event(self, event):
        self.mouseover = True
        Gtk.Button.do_enter_notify_event(self, event.crossing)
        pass

    def do_leave_notify_event(self, event):
        self.mouseover = False
        Gtk.Button.do_leave_notify_event(self, event.crossing)
        pass

    def do_button_press_event(self, event):
        self.mousedown = True
        Gtk.Button.do_button_press_event(self, event.button)

    def do_button_release_event(self, event):
        self.mousedown = False
        Gtk.Button.do_button_release_event(self, event.button)
...
class CairoPopup(Gtk.Window):
    ...
    def __init__(self, orient="down", no_arrow=False, type_="popup"):
        ...
        if visual is None:
            visual = gtk_screen.get_rgb_visual()
        ...
        #self.connect("draw", self.on_draw)
        #self.connect("enter-notify-event", self.do_enter_notify_event)
        #self.connect("leave-notify-event", self.do_leave_notify_event)
    ...
    def set_shape_mask(self):
        ...
        cr = cairo.ImageSurface(cairo.FORMAT_ARGB32, w, h)
        ctx = cairo.Context(cr)
        ctx.set_source_rgba(0, 0, 0,0)
        ctx.set_operator (cairo.OPERATOR_SOURCE)
        ctx.paint()
        r = int(self.popup_style.get("popup_roundness", 6))
        #rg = Gdk.cairo_region_create_from_surface(cr)
        if self.is_composited():
            make_path(ctx, 0, 0, w, h, r, 0,
                      self.__get_arrow_size(), self.pointer, self.ap)
            ctx.set_source_rgba(1, 1, 1, 1)
            ctx.fill()
            #self.get_window().input_shape_combine_region(rg, 0, 0)
        else:
            make_path(ctx, 0, 0, w, h, r, 1,
                      self.__get_arrow_size(), self.pointer, self.ap)
            ctx.set_source_rgb(0, 0, 0)
            ctx.fill()
            #self.get_window().shape_combine_region(rg, 0, 0)
        #del pixmap
        del cr
    ...
    def pointer_is_inside(self):
        rect = self.alignment.get_allocation()
        ax = rect.x; ay = rect.y; width = rect.width; height = rect.height
    ...
    def do_enter_notify_event(self, event):
        ...
        # FIXME: doesn't implement func
        #Gtk.Widget.do_enter_notify_event(self, event)


    def do_leave_notify_event(self, event):
        self._pointer_is_inside = False
        # FIXME: doesn't implement func
        #Gtk.Window.do_leave_notify_event(self, event.crossing)
...
class CairoArea(Gtk.Alignment):
    ...
    def __init__(self, text=None, area_type="window_item"):
        ...
        # fixme self. 인지 Gtk.Alignment인지 까먹음..
        Gtk.Alignment.set(self, 0, 0, 1, 1)
        self.popup_style = PopupStyle()
...
class CairoVBox(Gtk.VBox):
    ...
    def do_draw(self, event):
        ...
        for child in self.get_children():
            self.propagate_draw(child, event)
